...
| Bast's configured application-delivery path currently uses DuploCloud-backed service management. Migration away is the completion criterion; multitenant infrastructure with BastCare operating first is target state. | Draft for initial architecture review | Beth / Thanh | Owner direction recorded 2026-09-15; wording does not assert the target is already deployed and observed. |
